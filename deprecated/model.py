...
class Clipper(torch.nn.Module):
    def __init__(self, clip_variant='ViT-L/14', hidden_state=False, norm_embs=False, device=torch.device('cpu')):
        super().__init__()
        print(clip_variant, device)
        # clip_model, _, preprocess = open_clip.create_model_and_transforms(clip_variant, pretrained='laion2b_s32b_b82k')
    ...
    def embed_image(self, image):
        ...
        clip_emb = self.preprocess(image).unsqueeze(0).to(self.device)
...
class Voxel2Blip(torch.nn.Module):
    # define the prototype of the module
    def __init__(self, in_dim=39548, h=4096, n_blocks=4):
        super().__init__()
...
#         x = x.reshape(len(x), -1)
#         x = self.lin1(x)
#         x = self.norm(x.reshape(x.shape[0],4,48,48).contiguous())
#         # x = x.reshape(x.shape[0],4,48,48)
#         return x

# class Voxel2Blip(torch.nn.Module):
#     # define the prototype of the module
#     def __init__(self, in_dim=39548, h=2048, n_blocks=2):
#         super().__init__()

#         self.lin0 = nn.Sequential(
#             nn.Linear(in_dim, 4*h),
#             # nn.LayerNorm(4*h),
#             nn.ReLU(inplace=True),
#             nn.Dropout(0.2),
#         )
...
class Voxel2Blip(torch.nn.Module):
    # define the prototype of the module
    def __init__(self, in_dim=39548, h=4096, n_blocks=3):
        super().__init__()
...
class Voxel2Classification(torch.nn.Module):
    # define the prototype of the module
    def __init__(self, in_dim=39548,out_dim = 133, h=4096, n_blocks=2):
        super().__init__()
